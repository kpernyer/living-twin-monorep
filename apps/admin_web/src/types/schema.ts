...
export interface GoalEvent extends PubSubMessage<Partial<GoalData> & { old_status?: GoalStatus; new_status?: GoalStatus }> {
  event_type: 'goal.created' | 'goal.updated' | 'goal.status_changed' | 'goal.completed' | 'goal.deleted';
}

export interface DocumentEvent extends PubSubMessage<Partial<DocumentData> & { processing_duration_ms?: number }> {
  event_type: 'document.uploaded' | 'document.processing_started' | 'document.processing_completed' | 'document.processing_failed' | 'document.deleted';
}
...
export type UpdateEntityRequest<T> = Partial<Omit<T, 'id' | 'tenant_id' | 'created_at'>> & { updated_at: string };
