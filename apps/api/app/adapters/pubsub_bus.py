...
from google.cloud.pubsub_v1.types import DeadLetterPolicy, Duration, RetryPolicy
...
class PubSubBusAdapter:
    ...
    async def publish_event(
        ...
        try:
            ...
            # Prepare attributes
            message_attributes = {"tenant_id": tenant_id, "event_type": event_type}
            if attributes:
                message_attributes.update(attributes)

            # Publish the message
            future = self.publisher.publish(topic_path, message_bytes, **message_attributes)
    ...
    async def create_subscription(
        self, topic_name: str, subscription_name: str, filter_expression: Optional[str] = None
    ) -> bool:
    ...
    async def publish_domain_event(self, event: DomainEvent) -> Optional[str]:
        ...
        try:
            ...
            logger.info(
                f"Published event {event.event_type.value} for tenant "
                f"{event.tenant_id}: {message_id}"
            )
            return message_id
        ...
        try:
            ...
            request = {
                ...
                "retry_policy": RetryPolicy(
                    minimum_backoff=Duration(seconds=10), maximum_backoff=Duration(seconds=600)
                ),
            }
            ...
            if dlq_topic_path:
                request["dead_letter_policy"] = DeadLetterPolicy(
                    dead_letter_topic=dlq_topic_path, max_delivery_attempts=max_delivery_attempts
                )
            ...
            def message_callback(message):
                try:
                    ...
                    if event.tenant_id != tenant_id:
                        logger.warning(
                            f"Tenant mismatch: expected {tenant_id}, got {event.tenant_id}"
                        )
    ...
    async def publish_query_executed(
        self, query_id: str, tenant_id: str, user_id: str
    ) -> Optional[str]:
