...
from ..ports.authz import IAuthorizer
...
class FirestoreRepository:
    ...
    async def get_user(self, user_id: str, tenant_id: str) -> Optional[User]:
        ...
        try:
            doc_ref = self.db.collection('tenants').document(tenant_id).collection('users').document(user_id)
            doc = doc_ref.get()
    ...
    async def create_user(self, user: User) -> bool:
        """Create a new user."""
        try:
            doc_ref = self.db.collection('tenants').document(user.tenant_id).collection('users').document(user.id)
            doc_ref.set({
                'email': user.email,
                'name': user.name,
                'roles': user.roles,
