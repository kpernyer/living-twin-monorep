...
from ..domain.events import DomainEvent, EventType, EventFactory
...
class EventWorker:
    ...
    async def process_document_event(self, event: DomainEvent) -> bool:
        ...
        try:
            logger.info(f"Processing document event {event.event_type.value} for tenant {event.tenant_id}")
            ...
            if event.event_type == EventType.DOCUMENT_INGESTED:
                ...
                logger.info(f"Document {document_id} ({document_title}) ingested for tenant {event.tenant_id}")
    ...
    async def process_query_event(self, event: DomainEvent) -> bool:
        ...
        try:
            logger.info(f"Processing query event {event.event_type.value} for tenant {event.tenant_id}")
            ...
            if event.event_type == EventType.QUERY_EXECUTED:
                ...
                logger.warning(f"Query {query_id} failed for tenant {event.tenant_id}: {error_message}")
    ...
    async def process_user_event(self, event: DomainEvent) -> bool:
        ...
        try:
            logger.info(f"Processing user event {event.event_type.value} for tenant {event.tenant_id}")
    ...
    async def process_system_event(self, event: DomainEvent) -> bool:
        ...
        try:
            logger.info(f"Processing system event {event.event_type.value} for tenant {event.tenant_id}")
            ...
            if event.event_type == EventType.SYSTEM_ERROR:
                ...
                logger.error(f"System error for tenant {event.tenant_id}: {error_type} - {error_message}")
