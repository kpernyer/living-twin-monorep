...
from .agent_models import (
    Agent, AgentConfig, AgentResult, AgentExecution, 
    AgentCapability, AgentStatus, AgentType
)
...
class BaseAgent(ABC):
    ...
    async def run(self, isolation_mode: bool = False) -> AgentExecution:
        ...
        try:
            ...
            logger.info(f"Agent execution {self.execution_id} completed with {len(results)} results")
...
class NewsMonitoringAgent(BaseAgent):
    ...
    def validate_config(self, config: AgentConfig) -> bool:
        return (
            len(config.keywords) > 0 and
            config.update_frequency_minutes >= 5 and
            config.max_results_per_update <= 100
        )
    ...
    async def _fetch_demo_results(self) -> List[AgentResult]:
        ...
        return [
            AgentResult(
                ...
                content=f"This is a demo article monitoring keywords: {', '.join(self.agent.config.keywords)}",
...
class TechnologyTrendsAgent(BaseAgent):
    ...
    def validate_config(self, config: AgentConfig) -> bool:
        return (
            config.update_frequency_minutes >= 30 and  # Less frequent for global trends
            config.max_results_per_update <= 50
        )
    ...
    async def execute(self, isolation_mode: bool = False) -> List[AgentResult]:
        ...
        try:
            ...
            tech_keywords = [
                ...
                "blockchain", "cybersecurity", "digital transformation",
    ...
    async def _fetch_tech_trends(self, keywords: List[str]) -> List[AgentResult]:
        ...
        return [
            AgentResult(
                ...
                content="Recent developments in large language models show significant improvements in understanding context and generating human-like responses.",
            ...
            AgentResult(
                ...
                content="The cloud computing market continues to expand with hybrid and multi-cloud solutions gaining traction.",
...
class AgentFactory:
    ...
    def create_agent(agent: Agent) -> BaseAgent:
        ...
        if AgentCapability.NEWS_MONITORING in capabilities:
            ...
            raise ValueError(f"No agent implementation found for capabilities: {agent.capabilities}")
