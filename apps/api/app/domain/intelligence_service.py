"""Intelligence service for processing agent results and generating insights."""
...
from .intelligence_models import (
    ...
    PriorityLevel,
    PromptTemplate,
    StrategicAlignmentKPI,
    StrategicAlignmentQuery,
...
class IntelligenceService:
    ...
    def _initialize_default_templates(self):
        ...
        default_templates = [
            PromptTemplate(
                ...
                description="Extract fundamental strategic truths from market intelligence",
                role="ceo",
                category="strategic",
                template=(
                    "Analyze the following agent results and extract 3-5 fundamental "
                    "truths that impact our strategic position:\n\n"
                    "{agent_results}\n\n"
                    "Focus on:\n"
                    ...
                    "Format each truth as a clear, actionable statement with confidence "
                    "level and evidence count."
                ),
            ...
            PromptTemplate(
                ...
                category="technology",
                template=(
                    "From the following agent results, identify emerging technology "
                    "trends relevant to our industry:\n\n"
                    "{agent_results}\n\n"
                    "Analyze:\n"
            ...
            PromptTemplate(
                ...
                template=(
                    "Review the following agent results for critical issues requiring "
                    "immediate attention:\n\n"
                    "{agent_results}\n\n"
                    "Identify:\n"
    ...
    async def generate_intelligence(self, request: IntelligenceRequest) -> IntelligenceResponse:
        ...
        try:
            ...
            llm_response = await self._process_with_llm(
                prompt, template.output_format, request.analysis_depth
            )
    ...
    async def _get_agent_results(self, agent_ids: List[str], tenant_id: str) -> List[AgentResult]:
        ...
        for agent_id in agent_ids:
            ...
            results = await self.agent_service.get_agent_results(
                query=AgentResultQuery(agent_id=agent_id, tenant_id=tenant_id, limit=50)
            )
    ...
    def _build_prompt(
        self, template: PromptTemplate, agent_results: List[AgentResult], variables: Dict[str, Any]
    ) -> str:
...
"""
    ...
    async def _process_with_llm(
        self, prompt: str, output_format: str, analysis_depth: AnalysisDepth
    ) -> str:
...
"""
    ...
    async def _extract_truths(
        self, llm_response: str, tenant_id: str, category: str
    ...
    async def _extract_reports(
        self, llm_response: str, tenant_id: str, template: PromptTemplate
    ) -> List[CompiledReport]:
            ...
            for truth in truths:
                if truth.impact_level in [ImpactLevel.HIGH, ImpactLevel.CRITICAL]:
                    communication = CommunicationQueue(
                        ...
                        content=(
                            f"New strategic insight: {truth.statement}\n"
                            f"Confidence: {truth.confidence:.1%}\n"
                            f"Evidence: {truth.evidence_count} sources"
                        ),
            ...
            for report in reports:
                if report.priority in [PriorityLevel.HIGH, PriorityLevel.URGENT]:
                    communication = CommunicationQueue(
                        ...
                        content=(
                            f"New analysis report: {report.summary}\n"
                            f"Key insights: {len(report.insights)} items\n"
                            f"Recommendations: {len(report.recommendations)} actions"
                        ),
    ...
    async def _store_intelligence(
        ...
        logger.info(
            f"Stored {len(truths)} strategic insights, {len(reports)} reports, "
            f"{len(communications)} priority communications"
        )
    ...
    async def _get_users_by_role(self, tenant_id: str, role: str) -> List[Any]:
        ...
        return [{"id": f"user_{role}_1", "role": role}, {"id": f"user_{role}_2", "role": role}]
    ...
    async def get_communications(self, query: CommunicationQuery) -> List[CommunicationQueue]:
        ...
        user_communications = [
            c
            for c in self.communication_queue
        ...
        if query.priority_min:
            user_communications = [
                c for c in user_communications if c.priority >= query.priority_min
            ]
        ...
        if query.acknowledged is not None:
            user_communications = [
                c for c in user_communications if c.acknowledged == query.acknowledged
            ]
    ...
    async def calculate_strategic_alignment_scorecard(
        self, tenant_id: str
    ) -> StrategicAlignmentScorecard:
        ...
        risk_indicators = await self._identify_risk_indicators(
            tenant_id, alignment_kpis, execution_kpis
        )
        priority_interventions = await self._generate_priority_interventions(
            tenant_id, risk_indicators
        )
        ...
        scorecard = StrategicAlignmentScorecard(
            ...
            strategic_initiative_velocity=(
                alignment_kpis[0].current_value if alignment_kpis else 0.0
            ),
            goal_cascade_alignment=(
                alignment_kpis[1].current_value if len(alignment_kpis) > 1 else 0.0
            ),
            decision_strategy_consistency=(
                alignment_kpis[2].current_value if len(alignment_kpis) > 2 else 0.0
            ),
            resource_allocation_efficiency=(
                alignment_kpis[3].current_value if len(alignment_kpis) > 3 else 0.0
            ),
            strategic_response_time=execution_kpis[0].current_value if execution_kpis else 0.0,
            cross_functional_alignment=(
                execution_kpis[1].current_value if len(execution_kpis) > 1 else 0.0
            ),
            strategic_communication_effectiveness=(
                execution_kpis[2].current_value if len(execution_kpis) > 2 else 0.0
            ),
        ...
        logger.info(
            f"Calculated strategic alignment scorecard for tenant {tenant_id}: "
            f"{overall_score:.1f}% ({alignment_zone.value})"
        )
        return scorecard
    ...
    async def _identify_risk_indicators(
        self,
        tenant_id: str,
        alignment_kpis: List[StrategicAlignmentKPI],
        execution_kpis: List[StrategicAlignmentKPI],
    ) -> List[str]:
    ...
    async def _generate_priority_interventions(
        self, tenant_id: str, risk_indicators: List[str]
    ) -> List[str]:
    ...
    async def get_strategic_alignment_scorecard(
        self, query: StrategicAlignmentQuery
    ) -> StrategicAlignmentScorecard:
    ...
    async def get_strategic_alignment_history(
        self, query: StrategicAlignmentQuery
    ) -> List[StrategicAlignmentScorecard]:
