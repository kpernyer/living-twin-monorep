...
from .agent_models import (
    ...
    AgentCapability, AgentStatus, AgentType, AgentRequest,
    AgentExecutionRequest, AgentResultQuery, AgentHealthCheck
)
...
class AgentService:
        ...
        for agent_id, agent_results in self.results_cache.items():
            ...
            for result in agent_results:
                ...
                if query.date_to and result.created_at > query.date_to:
                    continue
                if query.keywords and not any(kw in result.keywords_matched for kw in query.keywords):
                    continue
    ...
    async def get_agent_health(self, agent_id: str) -> AgentHealthCheck:
        ...
        if recent_executions:
            completed_times = [e.execution_time_seconds for e in recent_executions if e.execution_time_seconds]
            if completed_times:
    ...
    def get_shared_agents(self) -> List[Agent]:
        ...
        return [
            agent for agent in self.agents.values()
            if agent.agent_type == AgentType.SHARED
        ]
