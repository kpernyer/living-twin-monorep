...
from ..adapters.firebase_auth import get_current_tenant, get_current_user
...
async def acknowledge_priority_communication(
    ...
    try:
        success = await intelligence_service.acknowledge_communication(
            communication_id, current_user.id
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to acknowledge communication: {str(e)}"
        )
...
async def get_templates(
    ...
    try:
        ...
        templates = [
            t for t in templates if t.tenant_id is None or t.tenant_id == current_tenant.id
        ]
...
async def get_strategic_intelligence_dashboard(
    ...
    try:
        ...
        recent_truths = await intelligence_service.get_truths(
            TruthQuery(tenant_id=current_tenant.id, limit=10)
        )

        # Get pending communications
        pending_communications = await intelligence_service.get_communications(
            CommunicationQuery(
                user_id=current_user.id, tenant_id=current_tenant.id, acknowledged=False, limit=10
            )
        )
...
async def get_intelligence_metrics(
    ...
    try:
        ...
        if date_from:
            truths_in_range = [
                t for t in intelligence_service.truths_cache.values() if t.created_at >= date_from
            ]
            total_truths = len(truths_in_range)

        if date_to:
            truths_in_range = [
                t for t in intelligence_service.truths_cache.values() if t.created_at <= date_to
            ]
            total_truths = len(truths_in_range)
        ...
        if intelligence_service.truths_cache:
            avg_confidence = sum(
                t.confidence for t in intelligence_service.truths_cache.values()
            ) / len(intelligence_service.truths_cache)
        else:
            avg_confidence = 0.0
...
async def setup_demo_strategic_intelligence(
    ...
    try:
        ...
        demo_request = IntelligenceRequest(
            ...
            variables={"industry": "technology", "company_size": "mid-size"},
            tenant_id=current_tenant.id,
            user_id="demo_user",
            priority="medium",
        )
...
async def get_strategic_alignment_scorecard(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get strategic alignment scorecard: {str(e)}"
        )
...
async def get_strategic_alignment_history(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get strategic alignment history: {str(e)}"
        )
...
async def calculate_strategic_alignment(
    ...
    try:
        scorecard = await intelligence_service.calculate_strategic_alignment_scorecard(
            current_tenant.id
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to calculate strategic alignment: {str(e)}"
        )
